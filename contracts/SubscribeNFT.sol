...
contract SubscribeNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("SubscribeNFT", "SSN") {
    ...
    modifier checkERC20(address addressERC20) {
        ...
        require(
            _allowERC20.get(addressERC20, "Unregistered ERC20 address") == 1,
    ...
    function getPriceETH(
        ...
        uint256 price = _priceETH[directionAddress].price > 0
            ...
            : (
                _priceETH[directionAddress].free
                    ? 0
                    : _priceETH[address(0)].price
            );
    ...
    function getPriceERC20(
        ...
        address directionAddress,
        uint256 amount
    ) public view checkERC20(paymentAddress) returns (uint256) {
        uint256 price = _priceERC20[paymentAddress][directionAddress].price > 0
            ? _priceERC20[paymentAddress][directionAddress].price
            : (
                _priceERC20[paymentAddress][directionAddress].free
                    ? 0
                    : _priceERC20[paymentAddress][address(0)].price
            );
    ...
    function setPriceERC20(
        ...
        address directionAddress,
        uint256 priceWei
    ) public onlyOwner checkERC20(paymentAddress) {
        _priceERC20[paymentAddress][directionAddress].price = priceWei;
        _priceERC20[paymentAddress][directionAddress].free = priceWei == 0
            ? true
            : false;
     ...
     */
    function setPriceByDirectionERC20(
        address paymentAddress,
        uint256 priceWei
    ) public checkERC20(paymentAddress) {
        _priceERC20[paymentAddress][msg.sender].price = priceWei;
        _priceERC20[paymentAddress][msg.sender].free = priceWei == 0
            ? true
            : false;
     ...
     */
    function getAllowERC20List()
        public
        view
        returns (AllowERC20Wrapper[] memory)
    {
    ...
    function mintWithERC20(
        ...
        address paymentAddress,
        uint256 amount
    ) public nonReentrant checkERC20(paymentAddress) {
        ...
        require(
            IERC20(paymentAddress).allowance(msg.sender, address(this)) ==
                price,
    ...
    function withdrawETH(address directionAddress, uint256 amount) internal {
        ...
        uint256 price = getPriceETH(directionAddress, amount);
        uint256 calculate = SafeMath.sub(
            price,
            fee,
    ...
    function withdrawERC20(
        ...
        if (calculate > 0) {
            IERC20(paymentAddress).transferFrom(
                msg.sender,
                directionAddress,
                calculate
            );
